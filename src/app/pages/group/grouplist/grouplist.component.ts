  ...
  loadgroup(){
    ...
    if (this.user) {
      if (this.user.roles && this.user.roles.includes('super')) {
        ...
        // gets the group that the admin administer and  
        // the groups that the admin belongs to but does not administer
        this.groupsService.getUserGroups(this.user.id).subscribe((userGroups) => {
          this.groupsService.getAdminGroups(this.user.id).subscribe((adminGroups) => {
            ...
            // checks if the is any duplication of groups and removes dupilcation
            ...
            // check if admins administers the group
            this.currentgrouplist.forEach(group => {
